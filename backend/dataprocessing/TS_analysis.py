...
def handle_missing_values(df):
    ...
    - If index is a DatetimeIndex (and not a MultiIndex), use time-based interpolation.
    - If index is a MultiIndex, fall back to linear interpolation (only method supported).
    - Otherwise, use default interpolation.
    """
    # Work only on numeric columns to avoid dtype issues (e.g. strings).
    ...
    # Datetime index (simple)
    if isinstance(df.index, pd.DatetimeIndex):
        # Forward-fill any missing index values without using the deprecated `method` argument.
        df.index = df.index.to_series().ffill()
        numeric_df_interp = numeric_df.interpolate(method="time")
    # MultiIndex: only linear interpolation is supported
    elif isinstance(df.index, pd.MultiIndex):
        numeric_df_interp = numeric_df.interpolate(method="linear")
    # Fallback for all other index types
    else:
        numeric_df_interp = numeric_df.interpolate()

    df[numeric_cols] = numeric_df_interp
    return df
...
def resample_data(df, freq='D'):
    ...
    # Pandas >= 2.2 deprecates 'M' in favor of 'ME' (month-end).
    # Normalize any legacy aliases here.
    normalized_freq = 'ME' if freq == 'M' else freq

    return df[value_col].resample(normalized_freq).mean().to_dict()
